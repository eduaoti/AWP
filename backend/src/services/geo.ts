...
export async function reverseGeocodeOSM(lat: number, lon: number): Promise<{ short?: string; full?: string } | null> {
    try {
  ...
  export async function reverseGeocodeGoogle(lat: number, lon: number): Promise<{ short?: string; full?: string } | null> {
    ...
    try {
      ...
      if (j.status === "OK" && j.results?.length) {
        ...
        const short = parts.length >= 2 ? `${parts[parts.length-2]}, ${parts[parts.length-1].split(" ").pop()}` : undefined;
   ...
   */
  export async function inferGeo(req: import("express").Request): Promise<{ lat?: number; lon?: number; accuracy_m?: number } | undefined> {
    const h = req.headers;
